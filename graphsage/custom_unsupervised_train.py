...
from graphsage.custom_utils import custom_load_data
...
flags.DEFINE_string('custom_data_folder', '', 'folder name of network data')
...
def log_dir(filename):
    log_dir = FLAGS.custom_data_folder + "/logs/unsup-" + filename.replace('.pickle', '')
...
def train(filename, train_data, test_data=None):
    G = train_data[0]
    ...
    summary_writer = tf.summary.FileWriter(log_dir(filename), sess.graph)
    ...
    if FLAGS.save_embeddings:
        ...
        save_val_embeddings(sess, model, minibatch, FLAGS.validate_batch_size, log_dir(filename))
        ...
        if FLAGS.model == "n2v":
            ...
            save_val_embeddings(sess, model, minibatch, FLAGS.validate_batch_size, log_dir(filename), mod="-test")
...
def main(argv=None):
    ...
    folder = FLAGS.custom_data_folder
    graphs_path = folder + 'graphs/'
    walks_path = folder + 'walks/'

    list_dir_graphs = sorted(os.listdir(graphs_path))
    list_dir_walks = sorted(os.listdir(walks_path))
    if len(list_dir_graphs) != len(list_dir_walks):
        print("number of graphs does not match number of walks!")
        return

    for i in range(len(list_dir_graphs)):
        filename = graphs_path + list_dir_graphs[i]
        walksname = walks_path + list_dir_walks[i]

        train_data = custom_load_data(filename=filename, walksname=walksname)
        print("Done loading training data..")
        train(list_dir_graphs[i], train_data)
